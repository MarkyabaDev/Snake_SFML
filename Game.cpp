...
Game::Game(float l_frameTime) : m_window("Snake", sf::Vector2u(800, 600)), m_snake(m_world.GetBlockSize(), &m_textbox),
                                m_world(sf::Vector2u(800, 600), &m_textbox), m_textbox(5, 14, 350, sf::Vector2f(225, 0))
{
...
void Game::Update()
{
    ...
    if (m_elapsedFixed.asSeconds() >= timeStep)
    {
        ...
        if (m_snake.HasLost())
        {
            ...
            m_textbox.Add("Snake length: " + std::to_string(m_snake.GetSnakeLength()));
        }
    }
}
